...
## sex vs. earn
ggplot(heights_df, aes(x=sex, y=earn)) +  geom_boxplot() + geom_point(position=position_dodge(width=0.75))
## race vs. earn
ggplot(heights_df, aes(x=race, y=earn)) +  geom_boxplot() + geom_point(position=position_dodge(width=0.75))

# https://ggplot2.tidyverse.org/reference/geom_bar.html
## Using `geom_bar()` plot a bar chart of the number of records for each `sex`
ggplot(heights_df, aes(sex)) + geom_bar(fill='red')

## Using `geom_bar()` plot a bar chart of the number of records for each race
ggplot(heights_df, aes(race)) + geom_bar(fill='red')

## Create a horizontal bar chart by adding `coord_flip()` to the previous plot
ggplot(heights_df, aes(race)) + geom_bar(fill='red') + coord_flip()
...
ggplot(data=florida_df, aes(x=date, group=1)) +  geom_line(aes(y = cases)) +  
...
ggplot(data=florida_df, aes(x=date, group=1)) +  geom_line(aes(y = cases), color = 'darkred') + 
...
ggplot(data=florida_df, aes(x=date, group=1)) +
  ...
  scale_colour_manual("", breaks = c('Florida', 'New York', 'California'), values = c('red', 'blue', 'green')) + xlab(" ") + ylab("Cases")
